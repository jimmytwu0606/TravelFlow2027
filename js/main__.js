...
/** 🧬 視覺回流：WYSIWYG 實境編修全量回寫發動機 (V2026.ULTRA 特徵感應版) */
syncCodeFromVisual() {
    ...
    try {
        ...
        // 🚀 2. 座標採樣：抓取所有具備編輯權限的節點 (H4, P, SPAN)
        const editables = Array.from(slot.querySelectorAll('[contenteditable="true"]'));
        if (editables.length === 0) return;

        // 💡 職人邏輯：內容特徵感應器
        const updateNodeByFeatures = (node) => {
            editables.forEach(el => {
                ...
                // B 軌道：費用感應 (純數字且 class 包含 pink/tabular)
                else if (/^\d+$/.test(text) && (cls.includes('theme-text-pink') || cls.includes('tabular-nums'))) {
                    node.expense = text;
                    if ('cost' in node) node.cost = Number(text) || 0;
                }
                // C 軌道：標題感應 (H4)
                else if (el.tagName === 'H4') {
                    if ('task' in node) node.task = text;
                    else if ('location' in node) node.location = text;
                    else if ('name' in node) node.name = text;
                }
                // D 軌道：描述感應 (P)
                else if (el.tagName === 'P') {
                    if ('spotlight' in node) node.spotlight = text;
                    else if ('note' in node) node.note = text;
                    else if ('info' in node) node.info = text;
                    else if ('move' in node) node.move = text;
                }
            });
        };
        ...
        if (Array.isArray(data)) {
            if (data.length > 0) updateNodeByFeatures(data[0]);
        } else if (data.stops && Array.isArray(data.stops)) {
            if (data.stops.length > 0) updateNodeByFeatures(data.stops[0]);
        } else {
            updateNodeByFeatures(data);
        }

        // 🚀 4. 數據固化：回寫 Textarea 並【同步至全域 State】
        const finalJson = JSON.stringify(data, null, 2);
        memoArea.value = finalJson;
        
        // 💡 關鍵修正：確保儲存按鈕點擊時能抓到新數據
        if (window.state && window.state.activeEditItem) {
            ...
            if (firstNode.time) window.state.activeEditItem.time = firstNode.time;
            if (firstNode.location || firstNode.task) {
                window.state.activeEditItem.location = firstNode.location || firstNode.task;
            }
        }

        // 🚀 5. 同步上方零件區 (使零件區 Input 即時反映視覺編輯)
        ...
        if (locInput && (firstNode.location || firstNode.task)) locInput.value = (firstNode.location || firstNode.task);
        if (costInput && firstNode.expense) costInput.value = firstNode.expense;

        console.log("✅ [Sync-Engine] 視覺編輯全量數據（含時間/費用）已固化");

    } catch (err) {
        console.error("❌ [Sync-Error] 回流邏輯斷路:", err);
    }
},
...
/** 🚀 景點/購物採集軌道：主控調度器 (V2026.ULTRA 物理導通版) */
promptEditSchedule(tripId, dayIndex, itemIndex = null) {
    ...
    // 🚀 核心修正 1：鎖定全域狀態，確保 syncStyleUI 抓得到 context
    state.activeTripId = tripId;
    ...
    // 🚀 核心修正 2：改用 requestAnimationFrame 確保 DOM 已經物理掛載
    requestAnimationFrame(() => {
        // 增加一個微小延遲確保 CSS 動畫不干擾 ID 抓取
        setTimeout(() => {
            console.log(`📡 [UI-Ignition] 啟動模式: ${item.style || 'default'}`);
            
            // 💡 關鍵：傳入整個 item 確保子函數能執行「數據回填」
...
/** 💾 數據固化中心：主控發動機 (V2026.ULTRA 作用域鎖定版) */
async saveScheduleData(tripId, dayIndex, itemIndex) {
    // 🚀 核心修正：將內部所有 this. 替換為 App. 確保物理導通
    const tripIndex = state.trips.findIndex(t => t.id === tripId);
    if (tripIndex === -1) return;

    try {
        // 🚀 1. 原子採集：使用 App 空間導通
        const rawData = App._captureScheduleFormData();
        
        // 🚀 2. 語義加工：使用 App 空間導通
        const processedNode = App._processNodeStorageByStyle(rawData);
        if (!processedNode) return;

        // 🚀 3. 物理焊接：使用 App 空間導通
        const targetTrip = state.trips[tripIndex];
        App._weldNodeToTripDay(targetTrip, dayIndex, itemIndex, processedNode, rawData);

        // 🚀 4. 磁區固化與 UI 重刷
        await App._finalizeScheduleStorage(targetTrip, tripId, dayIndex, rawData.style);

    } catch (e) {
        console.error("❌ [Sector-Fatal] 固化程序墜毀:", e);
        uiManager.showToast("⚠️", "數據格式異常，焊接中斷");
    }
},


/** 🧬 [Sub-Logic] 任務 A：物理層數據採集與洗滌 */
_captureScheduleFormData() {
    ...
    // 3. 核心洗滌：物理切除從 AI 複製時可能夾帶的不可見噪訊 (Zero-width chars)
    const rawMemo = (document.getElementById('sched-memo')?.value || "")
        .replace(/[\u200B-\u200D\uFEFF]/g, '')
        .trim();

    console.log(`📡 [Capture-Complete] Mode: ${style} | Memo-Length: ${rawMemo.length}`);
...
_processTransportStorage(memo) {
    ...
    // 🚀 [核心焊接 B]：將 UI 狀態回流進對象，作為 processTransportFuel 的燃料
    parsed.hubStation = uiHubStation || parsed.hubStation || "";
    parsed.hubReturn = isHubReturn;

    // 🚀 2. 邊際防禦：確保手動模式有起訖點
    const isHq = Array.isArray(parsed.stops) && parsed.stops.length > 0;
    if (!isHq && !routeData.start && !routeData.end) {
        uiManager.showToast("⚠️", "請輸入起訖站名以導通數據");
        return null;
    }

    // 🚀 3. 執行燃料處理器：生成包含 verifications (Google Maps 連結) 的最終 JSON 字串
    const finalMemoString = this.processTransportFuel(JSON.stringify(parsed), routeData);
    
    // 🚀 4. 數據歸一化與車資清算
    const finalLocation = (routeData.start && routeData.end) 
        ? `${routeData.start} → ${routeData.end}` 
        : (parsed.operator || "交通路網專案");

    // 💡 職人修正：直接調用全域物件方法，封殺 this 指向錯誤，並使用最新累算邏輯
    const finalCost = App.calculateCost ? App.calculateCost('transport', finalMemoString) : (Number(parsed.cost) || 0);
    ...
    return {
        ...
        memo: finalMemoString,      // 🚀 這裡包含了 Hub 資訊、里程碑與 Maps 連結
        cost: finalCost,            // 🚀 經過分段累算後的精準車資
        hubReturn: isHubReturn,     // 固化策略旗標
        hubStation: parsed.hubStation, // 固化大站指紋
        updatedAt: Date.now()
    };
},


/** ✨ 數據識別：行程燃料專屬軌道 (V2026.ULTRA 結構自適應版) */
_processItineraryStorage(memo) {
    ...
    try { 
        ...
        console.error("📋 [Storage-Error] 解析失敗:", e);
        uiManager.showToast('⚠️', "JSON 格式語法錯誤");
        return null; 
    }

    // 🚀 3. 結構分流優化：優先級 timeline > stops > array
    // 💡 職人診斷：確保抓到具備渲染燃料 (task/spotlight) 的那一層
    let stopArray = [];
    if (Array.isArray(parsed)) {
        stopArray = parsed;
    } else if (parsed.timeline && Array.isArray(parsed.timeline)) {
        stopArray = parsed.timeline; // 🎯 命中你剛才那包 JSON 的核心
    } else if (parsed.stops && Array.isArray(parsed.stops)) {
        stopArray = parsed.stops;
    }

    const firstItem = stopArray[0] || {};
    
    // 🚀 4. 封面提取
    const rawLocation = firstItem.task || firstItem.location || "景點燃料包";
    const cleanLocation = rawLocation.replace(/【.*?】/g, '').trim(); 

    // 🚀 5. 導通回傳：執行數據純化 (Sanitization)
    return {
        style: 'json',
        time: firstItem.time || "--:--",
        task: cleanLocation || rawLocation,    
        location: cleanLocation || rawLocation, 
        // 💡 關鍵修正：memo 不再存入原始大包，而是存入純化後的「純陣列」
        // 這樣 viewEngine 就能 100% 導通解析，不再因為外層包裹而斷路
        memo: JSON.stringify(stopArray), 
        cost: expenseManager.parseFuelCost(JSON.stringify(stopArray)) 
    };
},
...
/** 🚀 輔助工具：AI 指令動態合成 (V2026.ULTRA 戰術與實境加固版) */
...
【🚨 數據品質熔斷協定】
1. 🚀 空間導通：[note] 必須標註「出口編號/月台號碼」與「關鍵動線（如：手扶梯方位）」。
2. 🚀 避坑預判：針對長距離轉乘，必須提供「⚠️ 戰術提醒」或「📍 物理對位」（如：排隊人潮、步行分鐘、始發站建議）。
3. 🚀 噪音封殺：[stops] 僅限包含起點、終點、中繼景點、樞紐大站。嚴禁列出其餘站點。
4. 🚀 車資核實：[cost] 需包含所有段落費用（含特急/指定席料金）。
...
【⚠️ 物理約束】
- VISIT_STOP 必須執行抵達 (arr) 與再出發 (dep) 邏輯，嚴禁僅以站點路過。
- [hubStation] 欄位內容必須為: "${hubStation || '自動偵測最佳大站'}"。
- [spotlight] 必須總結該行程之「最高指導原則」。
`;
...
promptEditTransport(tripId) {
    ...
    const content = `
        <div class="space-y-4 text-left">
            <div id="flight-items-container" class="space-y-4">
                ${flights.length > 0 
            ...
            <button onclick="App.addFlightRow('${tripId}')" 
                    class="w-full py-4 border-2 border-dashed border-slate-200 rounded-[1.5rem] text-slate-400 font-black text-[11px] hover:bg-slate-50 transition-all flex items-center justify-center gap-2">
                <span>+</span> 新增航班設定
            </button>
        </div>`;

    const actions = `
        <button onclick="App.modalRemove('edit-transport-modal')" class="flex-1 py-4 bg-slate-100 text-slate-400 rounded-2xl font-black text-xs">取消</button>
        <button onclick="App.saveMultiTransportData('${tripId}')" class="flex-[2] py-4 theme-bg text-white rounded-2xl font-black text-xs shadow-lg">完成設定</button>`;
    ...
    renderHotelModal(trip) {
        const content = `
            <div class="space-y-6 text-left max-h-[65vh] overflow-y-auto no-scrollbar p-1" id="hotel-modal-body">
                ...
                <button onclick="App.addHotelInputRow()" class="w-full py-4 border-2 border-dashed border-slate-100 rounded-[1.5rem] text-[10px] font-black text-slate-400 hover:text-pink-500 transition-all">
                    + 新增另一間旅館配置
                </button>
            </div>`;

        const actions = `
            <button onclick="App.modalRemove('hotel-modal')" class="flex-1 py-4 bg-slate-100 text-slate-400 rounded-2xl font-black text-xs">取消</button>
            <button onclick="App.saveMultiHotelData('${trip.id}')" class="flex-[2] py-4 theme-bg text-white rounded-2xl font-black text-xs shadow-lg">確認配置</button>`;
...
/** 🏗️ 生成單列旅館輸入組件 (V2.ULTRA 數據增強版) */
generateHotelInputRow(index, trip, data = null) {
    const colorClass = this.hotelPalette[index % 5];
    return `
        <div class="hotel-row p-6 rounded-[2.5rem] border-2 ${colorClass} transition-all space-y-4 shadow-sm" data-index="${index}">
            ...
            <div class="space-y-3">
                <input type="text" class="hotel-name-input w-full bg-white border-none rounded-2xl p-4 font-bold text-sm focus:ring-2 focus:ring-pink-200 shadow-inner" 
                       placeholder="輸入飯店名稱" value="${data?.name || ''}">
                
                <div class="relative">
                    <input type="text" class="hotel-address-input w-full bg-white/60 border-none rounded-xl p-3 pl-9 text-xs focus:ring-1 focus:ring-pink-100 transition-all" 
                           placeholder="飯店地址" value="${data?.address || ''}">
                    <span class="absolute left-3 top-1/2 -translate-y-1/2 text-[10px]">📍</span>
                </div>

                <div class="relative">
                    <input type="tel" class="hotel-phone-input w-full bg-white/60 border-none rounded-xl p-3 pl-9 text-xs focus:ring-1 focus:ring-pink-100 transition-all" 
 ...
 * @param {boolean} silent - 是否為靜默模式
 */
async syncFromCloud(silent = false) {
    // 🛡️ 1. 物理攔截器
    if (!state.userId || state.userId === 'guest_sector' || !state.userProfile) {
        if (!silent) {
            uiManager.showToast('🔒', '請先登入以提領雲端存檔');
            this.navigateTo('backup');
        }
        return false;
    }
    ...
    try {
        ...
        if (result.status === 'SUCCESS') {
            // 💡 職人診斷：對位新版 fullDataPayload 結構
            // 包含：trips, backlogs, translations, config
            ...
            // 🛡️ 3. 版本時序衝突攔截
            if (!silent && cloudUpdatedAt < localLastEdit && localLastEdit !== 0) {
                const timeDiff = Math.round((localLastEdit - cloudUpdatedAt) / 1000 / 60);
                const confirmOverwrite = confirm(
                    `⚠️ 數據版本警示！\n\n雲端存檔版本較舊（早於本地 ${timeDiff} 分鐘）。\n確定要以雲端存檔覆蓋目前的本地配置嗎？`
                );
                if (!confirmOverwrite) return false;
            }

            // 🚀 4. 磁區解構與狀態焊接 (State Rehydration)
            state.trips = remoteData.trips || [];
            state.lastLocalEdit = cloudUpdatedAt;
            state.cloudStats = cloudMetadata;

            // 🚀 5. 全量物理固化程序 (IndexedDB Persistence)
            // A. 行程磁區同步 (內含購物與緊急資訊)
            await dbManager.saveAllTrips(state.trips);

            // B. 靈感精煉廠磁區同步 (Backlogs)
            if (remoteData.backlogs) {
                // 💡 職人作業：先清空本地磁區以執行全量覆蓋還原
                await dbManager.clear(dbManager.STORES.BACKLOG);
                await Promise.all(remoteData.backlogs.map(item => 
                    dbManager.put(dbManager.STORES.BACKLOG, item)
                ));
            }

            // C. 語料磁區同步 (Translations)
            const transData = remoteData.translations || remoteData.translationVault;
            if (transData) {
                await dbManager.clear(dbManager.STORES.TRANS_VAULT);
                await Promise.all(transData.map(item => 
            ...
            if (remoteData.config) {
                if (remoteData.config.theme) localStorage.setItem('tf_theme_key', remoteData.config.theme);
                if (remoteData.config.voice) localStorage.setItem('tf_voice_id', remoteData.config.voice);
            }
            ...
            // 7. 視圖重連：重啟初始化以確保全模組感應新數據
            if (typeof this.initSystem === 'function') {
                await this.initSystem(); // 執行深度初始化
            }
            
            this.navigateTo('backup');

            if (!silent) {
                uiManager.showToast('✈️', `已還原 ${state.trips.length} 筆行程與全量靈感`);
            }
            return true;
        } 
        ...
        if (result.status === 'EMPTY') {
            if (!silent) uiManager.showToast('ℹ️', '雲端磁區真空 (無存檔記錄)');
            state.cloudStats = { tripCount: 0, lastSync: null };
            localStorage.removeItem('tf_cloud_snapshot');
        } else {
            uiManager.showToast('❌', '雲端讀取異常');
        }
    } catch (err) {
...
/** 🚀 [Refinery-Core] 執行精煉投射 (V2026.ULTRA Dn 標籤即時同步版) */
App.executeRefineryProjection = async (idsJson, dayIndex) => {
    // 🚀 物理洗滌：還原從 HTML 傳入的 ID 陣列
    const ids = JSON.parse(idsJson.replace(/&quot;/g, '"'));
    ...
    try {
        // 1. 🚀 數據解析與洗滌
        const sanitizedJson = rawJson.replace(/```json/g, '').replace(/```/g, '').trim();
        const parsedData = JSON.parse(sanitizedJson);
        const nodesArray = Array.isArray(parsedData) ? parsedData : (parsedData.timeline || parsedData.stops || [parsedData]);

        const trip = state.trips.find(t => t.id === activeTripId);
        let finalNodesToPush = [];

        // 2. 🚀 模式分流發動機
        if (refineryMode === 'suite') {
            // 🎯 【整合模式】封裝為單一 ID 節點
            const firstItem = nodesArray[0] || {};
            finalNodesToPush = [{
                id: `node_${Date.now()}`,
                time: firstItem.time || "12:00",
                location: `【精煉套裝】${firstItem.task?.replace(/【.*?】/g, '') || '複合行程'}`,
                style: 'json',
            ...
            // 🎯 【單獨模式】拆解為獨立 ID 節點
            finalNodesToPush = nodesArray.map(node => ({
                ...node,
                id: `node_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
                style: 'json',
        ...
        // 3. 🚀 行程數據注入與固化
        trip.days[dayIndex].schedules.push(...finalNodesToPush);
        ...
        for (const id of ids) {
            ...
            if (backlog) {
                ...
                await dbManager.put(dbManager.STORES.BACKLOG, backlog);
            }
        }

        // 5. 🚀 物理回收與【即時重繪】
        modalEngine.remove('refinery-station-modal');
        uiManager.showToast('✨', `已投射至 D${dayIndex + 1} (${refineryMode === 'suite' ? '整合' : '單獨'})`);
        
        // 💡 關鍵焊接點：不要跳轉頁面，直接觸發 viewEngine 重新渲染精煉廠清單
        // 這能確保剛才寫入的行程數據被 _renderBacklogCard 掃描並顯示標籤
        const allBacklogs = await dbManager.getAll(dbManager.STORES.BACKLOG);
        viewEngine.renderBacklogPage(document.getElementById('main-content'), allBacklogs);

    } catch (e) {
        console.error("❌ [Refinery-Projection-Collapse]:", e);
        uiManager.showToast('⚠️', 'JSON 解析失敗，請檢查格式');
    }
};
...
App.filterBacklogByCity = (city) => {
    ...
    // 🚀 3. 執行視圖熱更新
    // 💡 職人診斷：直接調用 navigateTo('backlog')，這會讓 executeViewLogic 
    // 重新抓取 localStorage 並執行 renderBacklogPage 的過濾邏輯。
    App.navigateTo('backlog');
    
    console.log(`📡 [Backlog-Filter] 磁區已對焦至區域: ${city}`);
};
...
App.filterBacklogByCat = (cat) => {
    ...
    console.log(`📡 [Backlog-Filter] 磁區已對焦至屬性: ${cat}`);
};
