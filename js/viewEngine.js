...
_renderExportSelector() {
    ...
    const ids = Array.from(selectedCards).map(el => el.id.replace('card-', ''));
    ...
    <div class="space-y-6 text-left pb-4">
        ...
        <div class="p-5 bg-slate-50 rounded-[2rem] border border-slate-100">
            <div class="flex justify-between items-center">
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">待匯入總量</span>
...
_renderExportActions() {
    return `
        ...
        <button onclick="backlogManager.exportTargetBacklogs(Array.from(document.querySelectorAll('.backlog-card.selected')).map(el => el.id.replace('card-', '')))" 
...
/** 🧪 [Sub-Component] 渲染雙軌導航 Tabs - V2026.ULTRA 物理對焦強化版 */
    ...
    return `
        <div class="px-6 space-y-4 mb-2 animate-fade-in">
            ...
            <div id="backlog-cat-track" class="flex items-center gap-2 overflow-x-auto no-scrollbar pb-4 border-b border-slate-50 snap-x scroll-smooth">
                ...
                <button onclick="App.modalCreate('category-manager-modal', '🏷️ 標籤管理', viewEngine._renderCategoryEditor(), viewEngine._renderCategoryActions())"
                        class="shrink-0 w-8 h-8 bg-slate-50 text-slate-400 rounded-full flex items-center justify-center border border-dashed border-slate-200 active:scale-90 transition-all hover:border-pink-200 hover:text-pink-500">
